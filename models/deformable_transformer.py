...
class DeformableTransformerDecoder(nn.Module):
    def __init__(
        ...
        # replace class_emb with embedding classifier
...
class DeformableTransformer_CLIP_Q(nn.Module):
    def __init__(
    ...
    def get_outputs_class(self, embedding_classifier, data):
        ...
        embedding_classifier = F.normalize(embedding_classifier, dim=-1)
    ...
    def forward(
        ...
        decoder_mask=None, text_query=None, cache=None,
    ):
        ...
        if self.two_stage:
            ...
            embedding_classifier = torch.cat((self.decoder.embedding_classifier, self.decoder.bg_embed), dim=0)
            enc_outputs_class = self.get_outputs_class(embedding_classifier.to(output_memory.device), self.decoder.feature_align[self.decoder.num_layers](output_memory))
            ...
            reference_points = topk_coords_unact.sigmoid() # ([2, 300, 4])
            ...
            text_query = text_query.repeat_interleave(patch_num, 0)
            text_query = text_query.unsqueeze(0).expand(bs, -1, -1)
            tgt = tgt + text_query
...
def build_conditional_transformer(args):

    return DeformableTransformer_CLIP_Q(
