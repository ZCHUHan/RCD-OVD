...
class LvisDetection(TvLvisDetection):
    def __init__(self, img_folder, ann_file, transforms, return_masks, label_map,
                 prior_cats_file=None, is_train=False,
                 ):
...
def build(image_set, args):
    ...
    PATHS = {
        "train": (root, root / "lvis_v1_train_norare_proposal_freq.json"),
        "val": (root, root / "lvis_v1_val.json"),
    }

    _, ann_file = PATHS[image_set]
    img_folder = args.coco_path
    print('img_folder', img_folder, 'ann_file', ann_file)
    is_train=True if image_set=="train" else False
    prior_path = args.prior_novel_train_path if is_train else None
